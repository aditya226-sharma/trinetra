  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<PlainBadge cls="!text-violet-300">{entries.length} entries</PlainBadge>}
      />
      ...
      <form onSubmit={submit} className="glass flex flex-wrap items-end gap-3 p-4 anim-fadeup">
        {!isAdmin && (
          <div className="w-full rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2 text-[12px] text-violet-200">
            Read-only view — <span className="mono text-slate-200">admins</span> maintain the reference lists.
          </div>
        )}
        {isAdmin && (<>
        <label className="block">
          <span className="mono mb-1 block text-[10px] uppercase tracking-widest text-violet-300">entity kind</span>
          <select value={kind} onChange={(e) => setKind(e.target.value)} className="field mono px-3 py-2">
            {KINDS.map((k) => <option key={k} value={k}>{k}</option>)}
          </select>
        </label>
        <label className="block min-w-[180px] flex-1">
          <span className="mono mb-1 block text-[10px] uppercase tracking-widest text-violet-300">value</span>
        ...
        <label className="block min-w-[200px] flex-1">
          <span className="mono mb-1 block text-[10px] uppercase tracking-widest text-violet-300">reason</span>
        ...
        {msg && <span className="mono text-[11px] text-violet-500">✓ {msg}</span>}
      ...
      {entries.length === 0 ? (
        ...
        <div className="glass overflow-hidden anim-fadeup">
          <div className="flex items-center justify-between gap-2 border-b border-white/5 bg-black/40 px-4 py-2.5">
            <p className="mono text-[11px] tracking-widest text-violet-300">{tab} :: {entries.length}</p>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {entries.map((en, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-3">
                <span className="mono w-16 shrink-0 text-[10px] uppercase tracking-widest text-violet-300">{en.kind}</span>
                <span className={`mono min-w-0 flex-1 truncate text-[13px] ${en.active ? "text-slate-100" : "text-violet-300 line-through"}`}>{en.value}</span>
                <span className="hidden max-w-[220px] truncate text-[11px] text-violet-300 lg:block" title={en.reason}>{en.reason || "—"}</span>
                <span className="mono hidden shrink-0 text-[10px] text-violet-400 md:block">{en.created_by}</span>
                {isAdmin && (<>
                  <button onClick={() => toggle(en.kind, en.value, !en.active)} className={`chip mono !px-2 !py-1 text-[10px] ${en.active ? "chip-on" : ""}`}>
                    {en.active ? "ON" : "OFF"}
                  </button>
                  <button onClick={() => remove(en.kind, en.value)} className="mono text-[11px] text-pink-500 hover:text-pink-300">✕</button>
                </>)}
              </div>
            ))}
          </div>
